...
def _load_cached(data_dir: Path):
    ...
    if elections_cache.exists() and demos_cache.exists():
        ...
        print(f"  Elections: {len(elections)} rows, Demographics: {len(demos)} rows", flush=True)
        return elections, demos

    print("Cache not found — running full load (this is slow, but only once)...", flush=True)
...
def main():
    ...
    block_scores = legi_results.groupby(
        ['location', 'date_float', 'block']
    )['value'].sum().unstack(fill_value=0.0).reset_index()
    ...
    print(f"Block coverage: mean={known.mean():.1f}%, min={known.min():.1f}%, max={known.max():.1f}%",
          flush=True)
    ...
    targets = pd.merge(block_scores, abstentions, on=['location', 'date_float'], how='inner')
    ...
    print(f"Demographic indicators ({len(all_indicators)}): {all_indicators}", flush=True)
    ...
    for ind in all_indicators:
        df_ind = demos[demos['candidate'] == ind][['location', 'availability_date', 'value']].copy()
        df_ind = df_ind.rename(columns={'location': 'commune', 'value': ind})
        df_ind = df_ind.sort_values('availability_date').dropna(subset=['availability_date'])
        out_df = pd.merge_asof(
            out_df, df_ind,
            left_on='date_float', right_on='availability_date',
    ...
    # --- Geo coords ---
    geo_cols = legi_elections[['location', 'latitude', 'longitude']].drop_duplicates('location')
    ...
    available_indicators = [ind for ind in all_indicators
    ...
    print(f"After NaN cleanup: {out_df.shape} ({len(all_indicators)} indicators)", flush=True)
...
def _run_ridge(df, feature_cols, target_cols, val_date, alphas):
    ...
    train_df = df[(df['date_float'] < val_date) & ~np.isclose(df['date_float'], val_date, atol=1e-3)]
    ...
    print(f"  Train: {len(X_train)}, Val: {len(X_val)}, Features: {len(feature_cols)}", flush=True)
    if len(X_val) == 0:
    ...
    for t_col in target_cols:
        ...
        print(f"  {t_col}: Train R²={r2_tr:.4f}, Val R²={r2_v:.4f} (alpha={model.alpha_:.1f})", flush=True)
...
def _run_elastic_net(df, feature_cols, target_cols, val_date):
    ...
    for t_col in target_cols:
        model = ElasticNetCV(
            ...
            n_alphas=50, cv=5, max_iter=10000,
        )
        model.fit(X_tr, y_train[t_col])
        r2_tr = r2_score(y_train[t_col], model.predict(X_tr))
        r2_v = r2_score(y_val[t_col], model.predict(X_v))
        n_nonzero = np.sum(model.coef_ != 0)
        print(f"  {t_col}: Train R²={r2_tr:.4f}, Val R²={r2_v:.4f} "
              f"(alpha={model.alpha_:.4f}, l1_ratio={model.l1_ratio_:.2f}, "
              f"nonzero={n_nonzero}/{len(feature_cols)})", flush=True)
