...
// Nombres SIGNÉS, au moins typographique (U+2212). `toLocaleString` rend un trait d'union
// ASCII : sans ces deux helpers, le paragraphe sur l'erreur d'ancre mélangerait « −1,5 » pour
// le décalage et « -0,61 » pour les corrélations. Ils servent les seuls négatifs du texte.
const f1s = (x) => x == null ? "—" : (x > 0 ? "+" : x < 0 ? "−" : "±") + f1(Math.abs(x));
const f2s = (x) => x == null ? "—" : (x < 0 ? "−" : "") + f2(Math.abs(x));
