...
def _fit_gp(X_train, y_train, gp_cfg):
    n_comp = min(gp_cfg["n_components"], X_train.shape[0] - 1,
                 X_train.shape[1] * 3)
    nystroem = Nystroem(
        kernel=gp_cfg["kernel"], gamma=gp_cfg["gamma"],
        n_components=n_comp, random_state=gp_cfg["random_state"])
...
def run_gp_residual_boost(tc, train, val, feat_cols, demo_cols, national_est,
                          national_means, cfg, gp_cfg=None):
    ...
    for f_idx, held_mask in enumerate(fold_masks):
        ...
        if train_ok.sum() < 100:
            continue
        nystroem, bridge = _fit_gp(
            X_tr_gp[train_ok], oof_residuals[train_ok], gp_cfg)
    ...
    gp_oof_r2 = r2_score(y_true_train[ok], oof_combined[ok]) \
        if ok.sum() > 100 else ridge_oof_r2
    ...
    gp_val_pred, gp_val_std = _predict_gp(
        nystroem_final, bridge_final, X_v_gp)
...
def main():
    ...
    print(f"  GP: {GP_CONFIG['n_components']} Nystroem components, "
    ...
    for tc in TARGET_COLS:
        ...
        print(f"  Ridge+GP:    OOF={res['gp_oof_r2']:.4f}  "
        ...
        print(f"  GP calibrated uncertainty: median ±{med_std:.2f}pp  "
    ...
    for tc in TARGET_COLS:
        ...
        print(f"  {tc:20s} {r['ridge_val_r2']:7.4f} {r['gp_val_r2']:7.4f} "
