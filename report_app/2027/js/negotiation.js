...
function straddle() {
  ...
  return { p: NEG.rows.filter((r) => cross(r.p_lfi, pm)).length,
           q: NEG.rows.filter((r) => cross(r.q_lfi, lv) || cross(r.q_oth, lv)).length };
}
