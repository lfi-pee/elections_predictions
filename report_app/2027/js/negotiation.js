...
function negMethods() {
  ...
  const sp = d.split;
  $n("m-posture").innerHTML = `<b>Force réelle</b> : pour chaque circonscription, le modèle rejoue une gauche <b>divisée</b> (LFI seule contre le reste de la gauche, part nationale de LFI réglable de ${Math.round(+sp.shares[0] * 100)} à ${Math.round(+sp.shares[sp.shares.length - 1] * 100)} %, sondages : ${Math.round(sp.default_share * 100)} %, motif local du vote Mélenchon à la dernière présidentielle) et mesure la probabilité que LFI seule se qualifie au 2<sup>nd</sup> tour. À ${Math.round(sp.leverage_q * 100)} % ou plus, LFI n'a pas besoin de l'accord dans cette circonscription : sa revendication est incontestable, sa menace d'y aller seule crédible. <b>Force apparente</b> : l'écart au national du vote Mélenchon dans la gauche — ce que tout le monde voit sur la carte, sans modèle. <b>Postures</b> : <b>exiger</b> (en jeu, LFI seule se qualifie), <b>obtenir</b> (en jeu, pas seule), <b>monnaie d'échange</b> (imprenable mais LFI paraît forte : céder ne coûte rien et passe pour un sacrifice), <b>rien à jouer</b>. Le curseur « part de LFI dans la gauche » montre comment la force réelle bascule avec le niveau national de LFI.`;
