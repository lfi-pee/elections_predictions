...
2. **Abstention has no direct poll.** Solved with a gap-based turnout model (inter-election gap → national abstention, fit on 9 Legi+Pres training elections). Predicts 33.0% for 2024 (actual 31.0%, error 2.0pp). Extended cross-type training (20 dates) combined with PCA3 achieves R²=0.77 for Abstention. IMPORTANT: gap model must use Legi+Pres national means only — other types (Euro, Dept, Regi) have very different abstention dynamics and corrupt the gap-abstention relationship.
